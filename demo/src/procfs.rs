...
pub fn init_procfs(procfs:Arc<dyn VfsFsType>) -> Result<Arc<dyn VfsDentry>, Box<dyn Error>> {
    ...
    let p2_dt = root_dt.clone().insert("2", p2.clone())?;
    let p3_dt = root_dt.clone().insert("3", p3.clone())?;
    let p2 = p2.downcast_arc::<DynFsDirInodeImpl>().unwrap();
    let pp2 = p2.add_file_manually("pid", Arc::new(ProcessInfo::new(2)), "r--r--r--".into())?;
    p2_dt.insert("pid", pp2)?;
    let p3 = p3.downcast_arc::<DynFsDirInodeImpl>().unwrap();
    ...
    root_dt.clone().insert("meminfo", mem)?;
